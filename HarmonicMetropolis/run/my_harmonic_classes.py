...
class Basic_class:
    ...
    def calculate_contacts(self, sequence, method = 'explicit', return_plddt = False, return_trivial = False):
        ...
        if not return_trivial: 
            contact_map = self._eliminate_trivial_contacts(self, contact_map)
...
class Mutation_class(Basic_class):
    ...
    def __init__(
        ...
        if dx >= 0.: self.dx = dx
        else: raise ValueError("Mutation_class.__init__(): dx can't be negative.")
    ...
    def _get_id(self):
        ...
        dx_str = str(self.dx)
        s_str = str(self.seed)
        self.file_id = f'T{T_str}_dx{g_str}_s{s_str}'
    ...
    def metropolis(self, equilibration = False, print_start = True):
        ...
        if print_start:
            ...
            if self.generation == 0:
                print(f'{self.generation}\t{format(self.last_eff_energy, ".15f")}\t{format(self.last_ddG, ".15f")}\t{self.last_PAM1_distance}\t{self.last_Hamm_distance}\t{self.accepted_mutations}\t{self.T}\t{self.dx}\t{self.sequence_length}', file = data_file)
            else:
                print(f'{self.generation}\t{format(self.last_eff_energy, ".15f")}\t{format(self.last_ddG, ".15f")}\t{self.last_PAM1_distance}\t{self.last_Hamm_distance}\t{self.accepted_mutations / self.generation}\t{self.T}\t{self.dx}\t{self.sequence_length}', file = data_file)
        ...
        for imut in range(mutations):
            ...
            p = np.random.rand()
            if self.T > 0.:
                if p <= np.exp(- (eff_energy - self.last_eff_energy) / self.T  - (PAM1_distance - self.last_PAM1_distance) / self.dx):
                ...
                if eff_energy == 0. and p <= np.exp(- (PAM1_distance - self.last_PAM1_distance) / self.dx):
                    self.last_sequence = mt_sequence
                    self.last_eff_energy = eff_energy
                    self.last_ddG = ddG
                    self.last_PAM1_distance = PAM1_distance
