...
class Mutation_class(Basic_class):
    ...
    def __init__(
        ...
        if threshold >= 0. and threshold <= 1.: self.threshold = threshold
        else: raise ValueError("Mutation_class.__init__(): threshold must be included in [0, 1].")
    ...
    def _get_id(self):
        ...
        t_str = str(self.threshold)[:1] + str(self.threshold)[2:7]
        g_str = str(self.gamma)[:1] + str(self.gamma)[2:7]
        self.file_id = 'T' + T_str + '_t' + t_str + '_g' + g_str
    ...
    def _reset(self, _continue = False):
        if not _continue:
            self.last_sequence = self.ref_sequence
            self.last_contacts = self.ref_contacts
            self.last_eff_energy = self.calculate_effective_energy(self.ref_contacts)
            self.last_ddG = 0
    ...
    def _restart(self, _continue = False):
        # Find files
        ...
        if check:
            ...
            with open(f'../{self.results_dir}/data_{self.file_id}.dat', 'r') as data_file:
                ...
                self.threshold = float( last_line[6] )
                self.T = float( last_line[7] )
                self.gamma = float( last_line[8] )

        else:
            self._reset(_continue)
    ...
    def metropolis(self, equilibration = False):
        ...
        for imut in range(mutations):
            ...
            # Update lists
            p = np.random.rand()
            if p < np.exp( - (eff_energy - self.threshold) / self.T  - self.gamma * PAM1_distance ):
            ...
            # Save data
            print(f'{self.generation}\t{self.last_sequence}', file = mutants_file)
            print(f'{self.generation}\t{format(self.last_eff_energy, ".15f")}\t{format(self.last_ddG, ".15f")}\t{self.last_PAM1_distance}\t{self.last_Hamm_distance}\t{self.accepted_mutations / self.generation}\t{self.threshold}\t{self.T}\t{self.gamma}\t{len(self.ref_sequence)}', file = data_file)
    ...
    ### Multi-metropolis algorithm with gamma variation
    def multimetropolis(self, gammas = [], equilibration = True):
        # Convert input
        ...
        # Simple metropolis (unique gamma)
        if len(gammas) == 1:
            print(f'gamma: {gamma} (1/1)')
            self.set_gamma(gamma)
            self.set_restart_bool(self.restart_bool, _continue = False)
            if equilibration == True:
                self.metropolis(equilibration)
            self.metropolis()

        # Multi-metropolis (different gammas)
        elif len(gammas) > 1:
    ...
    def set_threshold(self, threshold):
        if threshold >= 0. and threshold <= 1.: self.threshold = threshold
        else: raise ValueError("Mutation_class.set_threshold(): threshold must be included in [0, 1].")
        self._get_id()
    ...
    def set_restart_bool(self, restart_bool, _continue = False):
        self.restart_bool = restart_bool
        if self.restart_bool: self._restart(_continue)
        else: self._reset(_continue)
