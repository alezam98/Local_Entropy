...
class Mutation_class(Basic_class):
    ...
    def __init__(
        ...
        if dmax >= 0.: self.dmax = dmax
        else: raise ValueError("Mutation_class.__init__(): dmax can't be negative.")
    ...
    def _restart(self):
        ...
        PAM1_distance, Hamm_distance = self.get_distances(self.last_sequence)
        assert PAM1_distance <= self.dmax, f"Not possible to start a simulation at distance {PAM1_distance}, which is outside the distance wall set by dmax {self.dmax}."
    ...
    def metropolis(self, equilibration = False, print_start = True):
        ...
        if print_start:
            ...
            if self.generation == 0:
                print(f'{self.generation}\t{format(self.last_eff_energy, ".15f")}\t{format(self.last_ddG, ".15f")}\t{self.last_PAM1_distance}\t{self.last_Hamm_distance}\t{self.accepted_mutations}\t{self.T}\t{self.dmax}\t{len(self.wt_sequence)}', file = data_file)
            else:
                print(f'{self.generation}\t{format(self.last_eff_energy, ".15f")}\t{format(self.last_ddG, ".15f")}\t{self.last_PAM1_distance}\t{self.last_Hamm_distance}\t{self.accepted_mutations / self.generation}\t{self.T}\t{self.dmax}\t{len(self.wt_sequence)}', file = data_file)
        ...
        for imut in range(mutations):
            ...
            # Update lists
            if PAM1_distance <= self.dmax:
                if self.T > 0.:
                    p = np.random.rand()
                    if p <= np.exp(- (eff_energy - self.last_eff_energy) / self.T):
                    ...
                    if eff_energy - self.last_eff_energy == 0.:
                        self.last_sequence = mt_sequence
                        self.last_eff_energy = eff_energy
                        self.last_ddG = ddG
                        self.last_PAM1_distance = PAM1_distance
            ...
            # Save data
            print(f'{self.generation}\t{self.last_sequence}', file = mutants_file)
            print(f'{self.generation}\t{format(self.last_eff_energy, ".15f")}\t{format(self.last_ddG, ".15f")}\t{self.last_PAM1_distance}\t{self.last_Hamm_distance}\t{self.accepted_mutations / self.generation}\t{self.T}\t{self.dmax}\t{len(self.ref_sequence)}', file = data_file)
    ...
    def print_status(self):
        ...
        if self.starting_sequence != self.wt_sequence and self.starting_sequence != self.ref_sequence:
            ...
            elif self.starting_sequence == self.ref_sequence:
    ...
    def set_dmax(self, dmax):
        if dmax >= 0.: self.dmax = dmax
        else: raise ValueError("Mutation_class.set_dmax(): dmax can't be negative.")
        self._get_id()
    ...
    def set_unique_length(self, unique_length):
        if unique_length >= 0: self.unique_length = unique_length
        else: raise ValueError("Mutation_class.__init__(): unique_length can't be negative.")
        self.restart_bool()
